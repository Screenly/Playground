...
//const stopId = '490005186S1'; //490005186S1 //490014050A  Replace with the ID of your desired bus stop
//40004402021A
//const apiKey = 'b4c24b35290947b089e509858e55f2f2'; // Replace with your TfL API Key
...
async function fetchBusData() {
	try {
		...
		console.log(lineData[bus1LineID]);
...
function getStatusInfo(route_status) {
	if (route_status === 0) {
			return { message: "Special Service", className: "onTime" };
	} else if (route_status === 1) {
			return { message: "Closed", className: "serviceClosed" };
	} else if (route_status === 2) {
			return { message: "Suspended", className: "serviceClosed" };
	} else if (route_status === 3) {
			return { message: "Part Suspended", className: "serviceClosed" };
	} else if (route_status === 4) {
			return { message: "Planned Closure", className: "serviceClosed" };
	} else if (route_status === 5) {
			return { message: "Part Closure", className: "serviceClosed" };
	} else if (route_status === 6) {
			return { message: "Severe Delays", className: "severeDelay" };
	} else if (route_status === 7) {
			return { message: "Reduced Service", className: "hasDelayed" };
	} else if (route_status === 8) {
			return { message: "Bus Service", className: "unknownStatus" };
	} else if (route_status === 9) {
			return { message: "Minor Delays", className: "hasDelayed" };
	} else if (route_status === 10) {
			return { message: "ON TIME", className: "onTime" };
	} else if (route_status === 11) {
			return { message: "Part Closed", className: "moderateDelay" };
	} else if (route_status === 12) {
			return { message: "Exit Only", className: "moderateDelay" };
	} else if (route_status === 13) {
			return { message: "No Step Free Access", className: "unknownStatus" };
	} else if (route_status === 14) {
			return { message: "Change of Frequency", className: "unknownStatus" };
	} else if (route_status === 15) {
			return { message: "Diverted", className: "unknownStatus" };
	} else if (route_status === 16) {
			return { message: "Not Running", className: "hasDelayed" };
	} else if (route_status === 17) {
			return { message: "Issues Reported", className: "hasDelayed" };
	} else if (route_status === 18) {
			return { message: "No Issues", className: "unknownStatus" };
	} else if (route_status === 19) {
			return { message: "No Status", className: "unknownStatus" };
	} else {
			return { message: "No Status", className: "unknownStatus" };
	}
}
