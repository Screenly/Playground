...
function generateQrCode(url, options, enableUtm, callback) {
  ...
  import_qrcode.default.toString(handleUrl(url), options, (err, result) => {
    if (err)
      throw err;
...
function setupBrandingColors() {
  ...
  const primaryColor = !settings.screenly_color_accent || settings.screenly_color_accent.toLowerCase() === "#ffffff" ? defaultPrimaryColor : settings.screenly_color_accent;
