...
function updateSensorCard(event: PeripheralEvent): void {
  const valueEl = document.querySelector<HTMLSpanElement>(`#val-${event.sensor}`)
  const unitEl = document.querySelector<HTMLSpanElement>(`#unit-${event.sensor}`)
  ...
  if (valueEl) {
    valueEl.textContent =
      typeof event.value === 'number' ? event.value.toFixed(2) : String(event.value)
  }
